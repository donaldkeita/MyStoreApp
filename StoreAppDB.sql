...
Select OrderID, CustomerID, ProductID, Location, Time FROM StoreApp.Orders WHERE CustomerID = 1002 ORDER BY Time

Select * FROM StoreApp.Orders WHERE OrderID = 6016

SELECT ProductID, ProductType, ProductName, Quantity, Cost FROM StoreApp.Product WHERE ProductID = (SELECT ProductID FROM StoreAPP.Orders WHERE CustomerID = @customerID);